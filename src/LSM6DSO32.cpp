...
uint8_t LSM6DSO32::set_fifo_watermark(short waterMarkBytes) { /// max watermark 511 (9 bits)
    ...
    uint8_t b = device->write_reg(REGISTER::FIFO_CTRL2, (byte)((waterMarkBytes>>8)&0x01)); // first bit of FIFO_CTRL2 is WTM8
    return (a | b); // if either writes fail, return true (1 means failure, 0 means success).
}

short LSM6DSO32::get_fifo_watermark() {
    return (short)(device->read_reg(REGISTER::FIFO_CTRL1) | (((device->read_reg(REGISTER::FIFO_CTRL2))&0x01)<<8));
}
...
short LSM6DSO32::get_BDR_counter_threshold() {
    return (short)(((device->read_reg(REGISTER::COUNTER_BDR_REG1)&0b00000111))<<8 | device->read_reg(REGISTER::COUNTER_BDR_REG2) );
}
